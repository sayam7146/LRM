...
const Login = ({ navigation }) => {
    ...
    const checksignin = async () => {
        console.log("pressed");
        const response = await fetch("http://192.168.0.110/LRM/api/user/checkuser?username=" + usertext + "&password=" + passwordtext);
        ...
        if (stcode == 200) {
            if (json[1] == 1) {
                console.log("ok")
                global.username = usertext;
                global.login = true;
                global.userid = json[0];
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.topsubcontainerimg}>
                ...
                <Text style={[styles.welcometext, { marginTop: 200, marginLeft: 80 }]}>User Login</Text>
            </View>

            <View style={styles.bottomsubcontainer}>
                <TextInput placeholder='Enter username' style={styles.inputfields}
